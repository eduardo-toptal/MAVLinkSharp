...
namespace MAVLinkBindings {
    ...
    public struct MagCalReportData : IMAVLinkMessageData {
        ...
        public int Write(byte[] p_buffer,int p_offset=0) {
            ...
            //byte[] b = p_buffer;
            //int    p = p_offset;
            MemoryMarshal.Write(b.Slice(p, 4), in Fitness                    ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in OfsX                       ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in OfsY                       ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in OfsZ                       ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in DiagX                      ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in DiagY                      ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in DiagZ                      ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in OffdiagX                   ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in OffdiagY                   ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in OffdiagZ                   ); p+=4;
            b[p++] = (byte)(CompassId);
            b[p++] = (byte)(CalMask);
            b[p++] = (byte)(CalStatus);
            b[p++] = (byte)(Autosaved);
            MemoryMarshal.Write(b.Slice(p, 4), in OrientationConfidence      ); p+=4;
            b[p++] = (byte)(OldOrientation);
            b[p++] = (byte)(NewOrientation);
            MemoryMarshal.Write(b.Slice(p, 4), in ScaleFactor                ); p+=4;
            return p;
        }
        #endregion
