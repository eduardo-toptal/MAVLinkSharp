...
namespace MAVLinkBindings {
    ...
    public struct TrajectoryRepresentationWaypointsData : IMAVLinkMessageData {
        ...
        public int Write(byte[] p_buffer,int p_offset=0) {
            ...
            for(int i=0;i<  5;i++) {
                MemoryMarshal.Write(b.Slice(p, 4), in VelX[i]          ); p+=4;
            }
            for(int i=0;i<  5;i++) {
                MemoryMarshal.Write(b.Slice(p, 4), in VelY[i]          ); p+=4;
            }
            for(int i=0;i<  5;i++) {
                MemoryMarshal.Write(b.Slice(p, 4), in VelZ[i]          ); p+=4;
            }
            for(int i=0;i<  5;i++) {
                MemoryMarshal.Write(b.Slice(p, 4), in AccX[i]          ); p+=4;
            }
            for(int i=0;i<  5;i++) {
                MemoryMarshal.Write(b.Slice(p, 4), in AccY[i]          ); p+=4;
            }
            for(int i=0;i<  5;i++) {
                MemoryMarshal.Write(b.Slice(p, 4), in AccZ[i]          ); p+=4;
            }
            for(int i=0;i<  5;i++) {
                MemoryMarshal.Write(b.Slice(p, 4), in PosYaw[i]        ); p+=4;
            }
            for(int i=0;i<  5;i++) {
                MemoryMarshal.Write(b.Slice(p, 4), in VelYaw[i]        ); p+=4;
            }
            for(int i=0;i<  5;i++) {
                b[p++] = (byte)(      Command[i]);
