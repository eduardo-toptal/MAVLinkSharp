...
namespace MAVLinkBindings {
    ...
    public struct LandingTargetData : IMAVLinkMessageData {
        ...
        public int Write(byte[] p_buffer,int p_offset=0) {
            ...
            MemoryMarshal.Write(b.Slice(p, 4), in AngleX             ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in AngleY             ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in Distance           ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in SizeX              ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in SizeY              ); p+=4;
            b[p++] = (byte)(TargetNum);
            b[p++] = (byte)(Frame);
