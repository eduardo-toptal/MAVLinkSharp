...
namespace MAVLinkBindings {
    ...
    public struct ControlSystemStateData : IMAVLinkMessageData {
        ...
        public int Write(byte[] p_buffer,int p_offset=0) {
            ...
            MemoryMarshal.Write(b.Slice(p, 4), in XVel             ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in YVel             ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in ZVel             ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in XPos             ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in YPos             ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in ZPos             ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in Airspeed         ); p+=4;
            for(int i=0;i<  3;i++) {
                MemoryMarshal.Write(b.Slice(p, 4), in VelVariance[i]   ); p+=4;
            }
            for(int i=0;i<  3;i++) {
                MemoryMarshal.Write(b.Slice(p, 4), in PosVariance[i]   ); p+=4;
            }
            for(int i=0;i<  4;i++) {
                MemoryMarshal.Write(b.Slice(p, 4), in Q[i]             ); p+=4;
            }
            MemoryMarshal.Write(b.Slice(p, 4), in RollRate         ); p+=4;
