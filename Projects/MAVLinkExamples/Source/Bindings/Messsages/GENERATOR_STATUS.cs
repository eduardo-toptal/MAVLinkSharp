...
namespace MAVLinkBindings {
    ...
    public struct GeneratorStatusData : IMAVLinkMessageData {
        ...
        public int Write(byte[] p_buffer,int p_offset=0) {
            ...
            MemoryMarshal.Write(b.Slice(p, 4), in BatteryCurrent             ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in LoadCurrent                ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in PowerGenerated             ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in BusVoltage                 ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in BatCurrentSetpoint         ); p+=4;
            b[p++] = (byte)(      Runtime);
