...
namespace MAVLinkBindings {
    ...
    public struct CameraTrackingImageStatusData : IMAVLinkMessageData {
        ...
        public int Write(byte[] p_buffer,int p_offset=0) {
            ...
            //byte[] b = p_buffer;
            //int    p = p_offset;
            MemoryMarshal.Write(b.Slice(p, 4), in PointX              ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in PointY              ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in Radius              ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in RecTopX             ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in RecTopY             ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in RecBottomX          ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in RecBottomY          ); p+=4;
