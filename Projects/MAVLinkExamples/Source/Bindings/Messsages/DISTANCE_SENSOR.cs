...
namespace MAVLinkBindings {
    ...
    public struct DistanceSensorData : IMAVLinkMessageData {
        ...
        public int Write(byte[] p_buffer,int p_offset=0) {
            ...
            b[p++] = (byte)(Covariance);
            MemoryMarshal.Write(b.Slice(p, 4), in HorizontalFov        ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in VerticalFov          ); p+=4;
            for(int i=0;i<  4;i++) {
                MemoryMarshal.Write(b.Slice(p, 4), in Quaternion[i]        ); p+=4;
            }
