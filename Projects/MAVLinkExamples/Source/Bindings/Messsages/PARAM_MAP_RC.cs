...
namespace MAVLinkBindings {
    ...
    public struct ParamMapRcData : IMAVLinkMessageData {
        ...
        public int Write(byte[] p_buffer,int p_offset=0) {
            ...
            //byte[] b = p_buffer;
            //int    p = p_offset;
            MemoryMarshal.Write(b.Slice(p, 4), in ParamValue0                    ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in Scale                          ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in ParamValueMin                  ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in ParamValueMax                  ); p+=4;
            b[p++] = (byte)(      ParamIndex);
