...
namespace MAVLinkBindings {
    ...
    public struct EstimatorStatusData : IMAVLinkMessageData {
        ...
        public int Write(byte[] p_buffer,int p_offset=0) {
            ...
            MemoryMarshal.Write(b.Slice(p, 4), in VelRatio               ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in PosHorizRatio          ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in PosVertRatio           ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in MagRatio               ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in HaglRatio              ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in TasRatio               ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in PosHorizAccuracy       ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in PosVertAccuracy        ); p+=4;
            b[p++] = (byte)(      Flags);
