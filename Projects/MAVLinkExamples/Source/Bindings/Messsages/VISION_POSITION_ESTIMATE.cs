...
namespace MAVLinkBindings {
    ...
    /// </summary>    
    public struct VisionPositionEstimateData : IMAVLinkMessageData {
        ...
        public float    X;                //Local X position
        public float    Y;                //Local Y position
        public float    Z;                //Local Z position
        public float    Roll;             //Roll angle
        public float    Pitch;            //Pitch angle
        public float    Yaw;              //Yaw angle
        public float[]  Covariance;       //Row-major representation of pose 6x6 cross-covariance matrix upper right triangle (states: x, y, z, roll, pitch, yaw; first six entries are the first ROW, next five entries are the second ROW, etc.). If unknown, assign NaN value to first element in the array.
