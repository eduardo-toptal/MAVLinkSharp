...
namespace MAVLinkBindings {
    ...
    public struct CameraTrackingGeoStatusData : IMAVLinkMessageData {
        ...
        public int Write(byte[] p_buffer,int p_offset=0) {
            ...
            MemoryMarshal.Write(b.Slice(p, 4), in HAcc                ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in VAcc                ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in VelN                ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in VelE                ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in VelD                ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in VelAcc              ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in Dist                ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in Hdg                 ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in HdgAcc              ); p+=4;
