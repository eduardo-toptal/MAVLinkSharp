...
namespace MAVLinkBindings {
    ...
    public struct OpenDroneIdLocationData : IMAVLinkMessageData {
        ...
        public int Write(byte[] p_buffer,int p_offset=0) {
            ...
            MemoryMarshal.Write(b.Slice(p, 4), in AltitudeBarometric      ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in AltitudeGeodetic        ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in Height                  ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), in Timestamp               ); p+=4;
            b[p++] = (byte)(      Direction);
