...
namespace MAVLinkBindings {
    ...
    public struct FollowTargetData : IMAVLinkMessageData {
        ...
        public int Write(byte[] p_buffer,int p_offset=0) {
            ...
            for(int i=0;i<  3;i++) {
                MemoryMarshal.Write(b.Slice(p, 4), in Vel[i]               ); p+=4;
            }
            for(int i=0;i<  3;i++) {
                MemoryMarshal.Write(b.Slice(p, 4), in Acc[i]               ); p+=4;
            }
            for(int i=0;i<  4;i++) {
                MemoryMarshal.Write(b.Slice(p, 4), in AttitudeQ[i]         ); p+=4;
            }
            for(int i=0;i<  3;i++) {
                MemoryMarshal.Write(b.Slice(p, 4), in Rates[i]             ); p+=4;
            }
            for(int i=0;i<  3;i++) {
                MemoryMarshal.Write(b.Slice(p, 4), in PositionCov[i]       ); p+=4;
            }
