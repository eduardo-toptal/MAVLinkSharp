...
namespace MAVLinkSharp.Bindings {
    ...
    public struct GimbalDeviceAttitudeStatusData : IMAVLinkMessageData {
        ...
        public float[]                 Q;                     //Quaternion components, w, x, y, z (1 0 0 0 is the null-rotation). The frame is described in the message description.
        public float                   AngularVelocityX;      //X component of angular velocity (positive: rolling to the right). The frame is described in the message description. NaN if unknown.
        public float                   AngularVelocityY;      //Y component of angular velocity (positive: pitching up). The frame is described in the message description. NaN if unknown.
        public float                   AngularVelocityZ;      //Z component of angular velocity (positive: yawing to the right). The frame is described in the message description. NaN if unknown.
        ...
        public float                   DeltaYaw;              //Yaw angle relating the quaternions in earth and body frames (see message description). NaN if unknown.
        public float                   DeltaYawVelocity;      //Yaw angular velocity relating the angular velocities in earth and body frames (see message description). NaN if unknown.
        public byte                    GimbalDeviceId;        //This field is to be used if the gimbal manager and the gimbal device are the same component and hence have the same component ID. This field is then set a number between 1-6. If the component ID is separate, this field is not required and must be set to 0.    
        ...
        */
        public void Init() {
            ...
            DeltaYaw                = default(float                 );
            DeltaYawVelocity        = default(float                 );
        ...
        public int Read(byte[] p_buffer,int p_offset=0) {
            ...
            DeltaYaw                = (float                 ) MemoryMarshal.Read<float >(b.Slice(p,4)); p+=4;
            DeltaYawVelocity        = (float                 ) MemoryMarshal.Read<float >(b.Slice(p,4)); p+=4;
        ...
        public int Write(byte[] p_buffer,int p_offset=0) {
            ...
            MemoryMarshal.Write(b.Slice(p, 4), ref DeltaYaw               ); p+=4;
            MemoryMarshal.Write(b.Slice(p, 4), ref DeltaYawVelocity       ); p+=4;
