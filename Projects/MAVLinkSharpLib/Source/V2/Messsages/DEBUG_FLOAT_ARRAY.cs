...
namespace MAVLinkSharp.Bindings {
    ...
    /// </summary>    
    public struct DebugFloatArrayData : IMAVLinkMessageData {

        /// <summary>
        /// Message Id Associated w/ this Struct
        /// </summary>    
        public int GetId() { return 350; }
        ...
        public ushort   ArrayId;      //Unique ID used to discriminate between arrays
        public char[]   Name;         //Name, for human-friendly display in a Ground Control Station
        ...
        */
        ...
        public int Read(byte[] p_buffer,int p_offset=0) {
            ...
            ArrayId        = (ushort) (b[p++] | LS8[b[p++]]);
            for(int i=0;i<10 ;i++) { Name[i]        = (char  ) (b[p++]); }
            for(int i=0;i<58 ;i++) { Data[i]        = (float ) MemoryMarshal.Read<float >(b.Slice(p,4)); p+=4; }            
            return p;
        }
        #endregion
        ...
        public int Write(byte[] p_buffer,int p_offset=0) {
            ...
            b[p++] = (byte)(      ArrayId);
            b[p++] = (byte)((int)ArrayId>>8 );
            for(int i=0;i< 10;i++) {
                b[p++] = (byte)(Name[i]);
            }
            for(int i=0;i< 58;i++) {
                MemoryMarshal.Write(b.Slice(p, 4), ref Data[i]       ); p+=4;
            }
            return p;
        }
        #endregion
