...
namespace MAVLinkSharp.Bindings {
    ...
    /// </summary>    
    public struct EscStatusData : IMAVLinkMessageData {

        /// <summary>
        /// Message Id Associated w/ this Struct
        /// </summary>    
        public int GetId() { return 291; }
        ...
        public int Read(byte[] p_buffer,int p_offset=0) {
            ...
            for(int i=0;i<4  ;i++) { Rpm[i]         = (int  ) (b[p++] | LS8[b[p++]] | LS16[b[p++]] | LS24[b[p++]]); }
            for(int i=0;i<4  ;i++) { Voltage[i]     = (float) MemoryMarshal.Read<float >(b.Slice(p,4)); p+=4; }
            for(int i=0;i<4  ;i++) { Current[i]     = (float) MemoryMarshal.Read<float >(b.Slice(p,4)); p+=4; }
            Index          = (byte ) (b[p++]);            
            return p;
        }
        #endregion
        ...
        public int Write(byte[] p_buffer,int p_offset=0) {
            ...
            for(int i=0;i<  4;i++) {
                b[p++] = (byte)(      Rpm[i]);
                b[p++] = (byte)((int)Rpm[i]>>8 );
                b[p++] = (byte)((int)Rpm[i]>>16);
                b[p++] = (byte)((int)Rpm[i]>>24);
            }
            for(int i=0;i<  4;i++) {
                MemoryMarshal.Write(b.Slice(p, 4), ref Voltage[i]    ); p+=4;
            }
            for(int i=0;i<  4;i++) {
                MemoryMarshal.Write(b.Slice(p, 4), ref Current[i]    ); p+=4;
            }
            b[p++] = (byte)(Index);
            return p;
        }
