...
namespace MAVLinkSharp.Bindings {
    ...
    public struct ActuatorControlTargetData : IMAVLinkMessageData {
        ...
        public ulong    TimeUsec= default(ulong);;     //Timestamp (UNIX Epoch time or time since system boot). The receiving end can infer timestamp format (since 1.1.1970 or since system boot) by checking for the magnitude of the number.
        ...
        public byte     GroupMlx=default(byte );;     //Actuator group. The "_mlx" indicates this is a multi-instance message and a MAVLink parser should use this field to difference between instances.    
        ...
        public int Write(byte[] p_buffer,int p_offset=0) {
            ...
            for(int i=0;i<  8;i++) {
                MemoryMarshal.Write(b.Slice(p, 4), ref Controls[i]   ); p+=4;
            }
            b[p++] = (byte)(GroupMlx);
            return p;
        }
